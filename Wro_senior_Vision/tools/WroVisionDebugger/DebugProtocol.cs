...
internal static class DebugCommandCodec
{
    public static byte[] CreateFlashCommand(uint sequence, byte brightness)
    {
        byte[] packet = new byte[13];
        packet[0] = 0xA5;
        packet[1] = 0x5A;
        packet[2] = 1;
        packet[3] = (byte)DebugPacketType.FlashCommand;
        packet[4] = 1;
        packet[5] = 0;
        packet[6] = (byte)(sequence & 0xFF);
        packet[7] = (byte)((sequence >> 8) & 0xFF);
        packet[8] = (byte)((sequence >> 16) & 0xFF);
        packet[9] = (byte)((sequence >> 24) & 0xFF);
        packet[10] = brightness;

        ushort crc = 0xFFFF;
        for (int index = 2; index <= 10; index++)
        {
            crc = UpdateCrc(crc, packet[index]);
        }

        packet[11] = (byte)(crc & 0xFF);
        packet[12] = (byte)(crc >> 8);
        return packet;
    }
...
internal sealed class GridMetadata
{
    ...
    public byte BorderScore { get; init; }
    public byte GridScore { get; init; }
    public byte ValidColorCells { get; init; }
    public byte RejectionCode { get; init; }
...
internal static class MetadataCodec
{
    ...
    public static bool TryDecode(ReadOnlySpan<byte> payload, out GridMetadata metadata)
    {
        ...
        metadata = new GridMetadata
        {
            ...
            BorderScore = payload[6],
            GridScore = payload[7],
            ValidColorCells = payload[8],
            RejectionCode = payload[9],
            Cells = cells
        };
        return true;
    }
